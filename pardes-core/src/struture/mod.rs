...
use syn::{parse_quote, parse_str, Field, Ident, ItemImpl, ItemStruct};
...
pub fn generate_debug_impl_core(fields: &[Field], struct_ident : &Ident) -> ItemImpl {
    ...
    let field_tokens: Vec<TokenStream> = fields
        ...
        .map(|(i, f)| {
            ...
            let ident: TokenStream = f.ident.clone()
            .map(|id| id.to_token_stream()).or_else(||Some(index)).unwrap();
            if let Some(name) = possible_name {quote! { .field(#name, &self.#ident)} }
            else {
                quote! { .field(&self.#ident)}
            }
        })
        .collect();
    let type_debug_method_token: TokenStream = parse_str(fields[0].ident.clone().map_or_else(||"debug_tuple",|_| "debug_struct")).unwrap();
    parse_quote! {
